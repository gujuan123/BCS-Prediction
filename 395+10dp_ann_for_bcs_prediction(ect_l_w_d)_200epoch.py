# -*- coding: utf-8 -*-
"""Chapter_3_395+29dp_ANN_for_BCS_Prediction(ECT_L_W_D)_200epoch.ipynb
...
from google.colab import files
uploaded = files.upload()
#WithHead_Real test (29 dimensions) with ECT=38
...
for i in range(70):
    #def pdb_test():
    # load the dataset1
    dataset1 = pd.read_csv('WithHead_Available data with common BCS dimension(ECT L W D).csv')
    X_train = dataset1.iloc[:, 0:4]
    y_train = dataset1.iloc[:, [4]]

    # load the dataset1
    dataset2 = pd.read_csv('WithHead_Real test (10 dimensions) with ECT=38.csv')
